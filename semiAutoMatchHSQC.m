function [H,matchInfo] = semiAutoMatchHSQC(ppm1,ppm2,ExpSpectra,Deep,ReferenceSpectra,HCdata,CompoundNames)
...
%By Ricardo Moreira Borges (10-16-2018)

disp(['matching data with ',CompoundNames])

Cq = find(HCdata==0);
[acq,~] = size(Cq); [a,~] = size(HCdata); A = a-acq;
%%%%%% Figure
figure,
contour(ppm1,ppm2,ExpSpectra,Deep,'k'), set(gca,'XDir','reverse'); set(gca,'YDir','reverse');
hold on, contour(ppm1,ppm2,ReferenceSpectra,'r'),set(gca,'XDir','reverse');set(gca,'YDir','reverse');
...
matchInfo = [num2str(aginput-1),' match of ', num2str(A),' from ',CompoundNames];
title([num2str(aginput-1),' match of ', num2str(A),' signals from ',CompoundNames],'Interpreter', 'none')
...
if (aginput-1)>(0.2*A)
  disp([num2str(aginput-1),' match of ', num2str(A),' signals from ',CompoundNames])
end
L=legend('data',CompoundNames);set(L,'Interpreter','none'); hold off
end
